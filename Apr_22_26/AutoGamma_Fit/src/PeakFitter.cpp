...
void PeakFitter::FitHistogram(TH1* h, TFile* fout) {
    ...
    for (int i = 0; i < nPeaks; i++) {
        ...
        if (time >= 0)
            tracker.Add(x0, time, peak_count, peak_counterr, sig, sigerr);
    }
    ...
    //     siggr->SetPoint(i, energies[i], 2.355*sigmas[i]);
    //     siggr->SetPointError(i, 0, 2.355*sigma_errs[i]);
    // }

    // siggr->SetName(Form("FWHM_vs_energy_%s", hname.c_str()));
    // siggr->SetTitle("FWHM vs Energy;Energy (keV);Sigma (keV)");
    // siggr->SetMarkerStyle(20);

    //  //sigma vs energy dir
    // sigmaDir->WriteTObject(siggr);
    // fout -> cd();
