...
void bckgrndsubfit3(const char* configfile="bckgrndsub_congfig.txt")
{
    FitConfig cfg;
   ...
   gamma -> Sumw2();
    ...
    TF1 *fit2 = new TF1("fit2", decay_model, cfg.Xmin, cfg.Xmax, 5);
    ...
    TFitResultPtr r1 = gamma->Fit(fit1,"SR");
    TFitResultPtr r2 = sub->Fit(fit2,"SR");
	...
	TF1 *p2 = new TF1("p2", decay_parent, cfg.Xmin, cfg.Xmax, 2);
	TF1 *b2 = new TF1("b2", decay_parent, cfg.Xmin, cfg.Xmax, 2);
	TF1 *c2 = new TF1("c2","[0]",  cfg.Xmin, cfg.Xmax);

	p2->SetParameters(fit2->GetParameter(0),fit2->GetParameter(1));
	b2->SetParameters(fit2->GetParameter(2),fit2->GetParameter(3));
